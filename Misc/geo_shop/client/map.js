  ...
  */
  ...
  var Searcher = function(prefs) {
    ...
    this.search = function(cb) {
      ...
      url += '?total_products=' + this.prefs.count;
      url += '&lat=' + this.prefs.position.lat;
      url += '&lng=' + this.prefs.position.lng;
      url += '&radius=' + this.prefs.radius;
      if(this.prefs.tags.length)
        url += '&tags=' + this.prefs.tags.join(',');
      $.getJSON(url).done(function (data) {
        return cb(null, data.products);
      }).fail(function(jqxhr, textStatus, error) {
        return cb(error || textStatus || "Request Failed");
      })
    };
  };
